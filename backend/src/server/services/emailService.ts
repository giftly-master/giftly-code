...
export async function sendAdminAlert(data: {
  ...
  const mailOptions = {
    from: `"Giftly Security" <${EMAIL_CONFIG.auth.user}>`,
    to: adminEmail,
...
export async function sendVerificationEmail(
  ...
  const mailOptions = {
    from: `"Giftly" <${EMAIL_CONFIG.auth.user}>`,
    to: email,
    subject: "Verify Your Email - Giftly",
    html: generateEmailTemplate(otp, userName),
    text: `Your Giftly verification code is: ${otp}\n\nThis code will expire in 10 minutes.\n\nIf you didn't request this code, please ignore this email.`,
  };
...
export async function sendForgotPasswordEmail(
  ...
  const mailOptions = {
    from: `"Giftly" <${EMAIL_CONFIG.auth.user}>`,
    to: email,
    subject: "Reset Your Password - Giftly",
    html: generateForgotPasswordTemplate(resetLink, userName),
    text: `Reset your Giftly password by clicking this link: ${resetLink}\n\nThis link will expire in 1 hour.\n\nIf you didn't request a password reset, please ignore this email.`,
  };
...
export async function sendPasswordResetConfirmationEmail(
  ...
  const mailOptions = {
    from: `"Giftly" <${EMAIL_CONFIG.auth.user}>`,
    to: email,
    subject: "Password Changed - Giftly",
    html: generatePasswordResetConfirmationTemplate(userName),
    text: `Your Giftly password has been successfully changed.\n\nIf you did not perform this action, please contact support immediately.`,
  };
  ...
  return generateBaseTemplate({
    ...
    content: `
      <p style="margin: 0 0 20px; color: #4a5568; font-size: 16px; line-height: 1.6;">
        Thank you for signing up with Giftly! To complete your registration, please verify your email address using the code below:
      </p>
...
function generatePasswordResetConfirmationTemplate(userName?: string): string {
  return generateBaseTemplate({
    ...
    content: `
      <p style="margin: 0 0 20px; color: #4a5568; font-size: 16px; line-height: 1.6;">
        This is a confirmation that your Giftly password has been successfully changed.
      </p>
      
      <p style="margin: 20px 0; color: #4a5568; font-size: 16px; line-height: 1.6;">
        If you did not perform this action, please contact our security team immediately at support@Giftly.com.
      </p>
  ...
  return generateBaseTemplate({
    ...
    content: `
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin: 30px 0;">
        <tr>
          <td align="center">
            <a href="mailto:support@Giftly.com" style="background-color: #f7fafc; border: 1px solid #e2e8f0; color: #4a5568; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 500; font-size: 14px; display: inline-block;">
              Contact Support
            </a>
          </td>
        </tr>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          <tr>
            <td style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px 20px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 700;">Giftly</h1>
...
export async function sendGiftConfirmationOTP(
  ...
  const mailOptions = {
    from: `"Giftly" <${EMAIL_CONFIG.auth.user}>`,
    to: email,
    subject: "Gift Confirmation OTP - Giftly",
    html: generateEmailTemplate(otp, userName),
...
export async function sendGiftNotificationToRecipient(
  ...
  const content = `
    ...
    <p style="margin: 20px 0 0; color: #718096; font-size: 14px; line-height: 1.6;">
      Log in to your Giftly account to view and manage your gift. Thank you for using Giftly!
    </p>
  `;
